...
async function initApp() {
    ...
    } else if (activeSection === 'search-results') {
        showTab('search-results', true);
        const urlParams = new URLSearchParams(window.location.search);
        const q = urlParams.get('q');
        if (q) {
            const searchInputs = document.querySelectorAll('.sidebar-search-input');
...
function switchTimeline(type, fromHashChange = false) {
    ...
    if (!tabFeed && !fromHashChange) {
        const base = window.KUTSOCIAL_BASE_PATH || '';
        window.location.href = base + '/' + type;
        return;
    }
...
async function viewProfile(accountId, fromHashChange = false) {
    ...
    if (!document.getElementById('tab-profile-view')) {
        const base = window.KUTSOCIAL_BASE_PATH || '';
        window.location.href = base + '/@id-' + accountId;
        return;
    }
...
async function viewTootThread(statusId, fromHashChange = false) {
    ...
    if (!document.getElementById('tab-thread-view')) {
        const base = window.KUTSOCIAL_BASE_PATH || '';
        window.location.href = `${base}/statuses/${statusId}`;
        return;
    }
    
    activeThreadId = statusId;
    const base = window.KUTSOCIAL_BASE_PATH || '';
    const threadPath = `${base}/statuses/${statusId}`;
...
async function performSearch(query) {
    ...
    if (!document.getElementById('tab-search-results')) {
        const base = window.KUTSOCIAL_BASE_PATH || '';
        window.location.href = base + '/search-results?q=' + encodeURIComponent(query);
        return;
    }
...
async function loadFollowedHashtags() {
    if (!document.getElementById('tab-followed-hashtags')) {
        const base = window.KUTSOCIAL_BASE_PATH || '';
        window.location.href = base + '/followed-hashtags';
        return;
    }
